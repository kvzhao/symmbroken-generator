...
from data import cal_energy

# model
tf.app.flags.DEFINE_integer('model_dim', 256, 'Dimensionality of the model')
...
# dataset
tf.app.flags.DEFINE_integer('L', 32, '(Can not be modified) System size of Ising model')
...
def save_image(name, spins):
    engs = []
    for s in spins:
        engs.append(cal_energy(s))
    weights = np.ones_like(engs) /float(len(engs))
    plt.hist(engs, bins=100, weights=weights)
    plt.xlabel('energy')
